...
class BookList extends React.Component {
    ...
    render() {
        const imageAddress = `http://covers.openlibrary.org/b/isbn/${this.state.book.isbn[3]}-M.jpg`
        
        return (
            <div className='book-list'>
                <BookCard 
                bookTitle = { this.state.book.title }
                author = { this.state.book.author_name[0] }
                publishedYear = { this.state.book.first_publish_year }
                publishedBy = { this.state.book.publisher[0] }
                genre = { this.state.book.subject[0] }
                ISBN = { this.state.book.isbn[0] }
                cover = { imageAddress }
                />
            </div>
        )
